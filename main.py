...
def get_ai_response(prompt):
    ...
    # الخطة (ب) المضمونة 100%: خوادم المعالجة اللغوية الحرة والمفتوحة بدون مفاتيح المقارنة (Pollinations & Llama System)
...
def build_file_doc(message, path_str):
    ...
    # استخلاص التسمية من الشرح النصي المرفق إن وجد
...
def process_media_group(chat_id, media_group_id, path_str):
    # وقت الانتظار الحرج لتجميع باقات الوسائط المتتالية من تيليجرام
    time.sleep(3)
...
def show_menu(chat_id):
    ...
    # توجيه المشرفين أثناء الإجراءات الهيكلية الخاصة
    ...
    # بناء الصفحة الرئيسية الخالية من العيوب
    if not path:
        for key in ACADEMIC_STRUCTURE.keys():
            markup.add(KeyboardButton(key))
        markup.add("👨‍💻 تواصل مع المشرف العام")
        
        # ميزات التحكم الإداري الأعلى (تظهر للواثق فقط في حالة عدم تفعيل وضع تجربة طالب)
    ...
    # عرض محتوى مجلد ميزات الطالب المطور والمعزول بالكامل
    ...
    # توليد محتويات الأقسام العادية والديناميكية المخزنة في MongoDB
    ...
    # إرفاق أدوات الإدارة الحركية للمشرفين المسؤولين عن هذا المسار المخصص
...
def send_file_to_user(chat_id, res, has_perm):
    try:
        ...
        # معالجة التدفق الفعلي للملفات دون فقدان أو تحول لرسائل فارغة
        if file_type == 'text':
...
def universal_handler(message):
    ...
    # وضع الأرشفة التلقائية في المجموعات والقنوات المشتركة
    if message.chat.type in ['group', 'supergroup']:
        auto_archive_handler_logic(message)
        if message.content_type != 'text' or not text.startswith("/"):
            return

    # [1] نظام الاستقبال الاحترافي والذكي للمستندات والملفات المتعددة (الألبومات وباقات الـ PDF)
    ...
    if text == "🤖 المساعد الذكي (AI)":
        ...
        bot.send_message(chat_id, "🤖 المساعد الذكي واللغوي الفعال جاهز لمساعدتك الآن!\nاطرح سؤالك العام أو الأكاديمي أو البرمجي وسأجيبك فوراً ودون انقطاع:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر"))
        return
    ...
    if mode == "search" and text:
        ...
        if results:
            ...
            bot.send_message(chat_id, "❌ نعتذر، لم نجد أي ملفات مطابقة للكلمة المدخلة في الأرشيف الأكاديمي الحالي.")
        reset_modes(chat_id)
        show_menu(chat_id)
        return
    ...
    # [3] منظومة التنبيهات المخصصة للمقررات والمواد الدراسية
    ...
    if is_super_admin(chat_id):
        if text == "👥 إحصائيات المشتركين":
            ...
            # حماية الرسالة من تجاوز الحد الحرج لحجم رسائل تيليجرام (4096 حرف)
    ...
    if mode == "rename_folder_action" and text:
        ...
        user_path[chat_id][-1] = text.strip()  # تحديث المسار الحي لتفادي التجميد أو حدوث ثغرات استدعاء
