...
def send_file_to_user(chat_id, res, has_perm):
    try:
        ...
        # رابط تليجرام الداخلي المباشر السريع لفتح المجلد
        deep_folder_url = f"https://t.me/{BOT_USERNAME}?start=folder_{file_id_str}"

        markup = InlineKeyboardMarkup(row_width=2)

        # 1. أزرار التحكم والإدارة الحصرية (تظهر للمشرفين والمالك فقط)
        if has_perm and not testing_mode.get(chat_id):
            ...
            # 🔥 خدعة المالك: زر النشر المباشر في قنوات وجروبات الدفعة دون خسارة الأزرار الشفافة
            markup.add(InlineKeyboardButton("📢 نشر في قنوات/جروبات الدفعة", switch_inline_query=f"file_{file_id_str}"))
        ...
        # 3. خوارزمية تسمية الزر الشفاف للمجلد (القسم - المقرر)
        path_str = res.get('menu_path', '')
        btn_name = "📁 المجلد الرئيسي" 
        if path_str:
            parts = path_str.split(' > ')
        ...
        # إدراج زر المجلد الشفاف مباشرة أسفل الملف للمشرفين فقط
        if has_perm and not testing_mode.get(chat_id):
            markup.add(InlineKeyboardButton(btn_name, url=deep_folder_url))

        # 4. إعداد بيانات الملف والـ Caption المحمي للطلاب العاديين
        file_type = res.get('type', 'document')
        file_id = res.get('file_id')
        base_name = res.get('name', 'وثيقة')
        up_date = res.get('upload_date', datetime.utcnow()).strftime('%Y-%m-%d')
        ...
        # الأمان الاحتياطي: الأمر النصي الأزرق الصامد دائماً عند تحويل الطلاب للملف في الجروبات
        caption += f"\n\n📥 لفتح المجلد الأكاديمي مباشرة بلمحة عين:\n/start folder_{file_id_str}"

        # 5. الإرسال المباشر والنظيف للمستخدم
        if file_type == 'text': 
