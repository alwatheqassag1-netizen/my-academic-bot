...
if settings_col.count_documents({}) == 0:
    settings_col.insert_one({
        "status": "active",
        "emergency_flags": {"ai": False, "upload": False, "search": False, "ads": False}
    })
...
# ==========================================
# 4. الهيكل الأكاديمي الأساسي (بدون تغيير)
# ==========================================
ACADEMIC_STRUCTURE = {
    "🌱 مستوى أول": {
        "📅 ترم أول": {},
...
# ==========================================
# 5. دوال التحكم والصلاحيات الديناميكية
# ==========================================
...
def log_action(admin_id, action_type, details):
    try:
        admin_name = users_col.find_one({"chat_id": admin_id}).get("first_name", "Admin")
...
# ==========================================
# 6. الذكاء الاصطناعي والخلفية (خفيف جداً)
# ==========================================
    ...
    if GEMINI_API_KEY and not GEMINI_API_KEY.startswith("AIzaSy"):
        for model_name in ["gemini-2.0-flash-lite-preview-02-05", "gemini-2.0-flash", "gemini-1.5-flash"]:
            try:
                ...
                res = requests.post(url, json={"contents": [{"parts": [{"text": prompt}]}], "generationConfig": {"temperature": 0.4, "maxOutputTokens": 600}}, headers={'Content-Type': 'application/json'}, timeout=7)
                if res.status_code == 200: return res.json()['candidates'][0]['content']['parts'][0]['text'].strip()
            except: continue
    for backup_model in ["openai", "llama", "mistral"]:
        try:
            res = requests.get(f"https://text.pollinations.ai/{requests.utils.quote(prompt)}?model={backup_model}&seed=42", timeout=12)
    ...
    while True:
        try:
            ...
            for r in list(reminders_col.find({"notify_at": {"$lte": now}})):
                try: bot.send_message(r['chat_id'], f"⏰ *تنبيه شخصي:*\n\n{r['text']}", parse_mode="Markdown")
                except: pass
...
# ==========================================
# 7. إدارة الملفات المتتالية (تصاعدياً مع sort_order)
# ==========================================
...
def process_media_group(chat_id, media_group_id, path_str):
    ...
    messages_batch.sort(key=lambda msg: msg.message_id) 
    ...
    for msg in messages_batch:
        ...
        if doc['file_id']:
            if not files_col.find_one({"menu_path": path_str, "file_id": doc['file_id']}):
                ...
                succ += 1
                time.sleep(0.05) 
    try: bot.send_message(chat_id, f"✅ تم إدراج الدفعة بالترتيب تصاعدياً!\n📦 عدد الملفات المحفوظة: {succ}\n📁 في المسار: {path_str}")
    except: pass
...
def start_command(message):
    ...
    if settings.get("status") == "inactive" and not is_any_admin(chat_id):
        bot.send_message(chat_id, "🚧 البوت حالياً تحت التحديث والصيانة. نعود إليكم قريباً."); return
...
# ==========================================
# 9. ديناميكية القوائم (Dynamic Menus)
# ==========================================
def show_menu(chat_id):
    ...
    if not path:
        for key in ACADEMIC_STRUCTURE.keys(): 
            if "مستوى" in key: markup.add(KeyboardButton(key))
                
        markup.add(KeyboardButton("🌟 ميزات الطالب"), KeyboardButton("📖 دليل القسم"))
        markup.add(KeyboardButton("📞 التواصل مع المشرف العام"))
        
        if is_super_admin(chat_id) and not testing_mode.get(chat_id):
            markup.add(KeyboardButton("👑 لوحة المشرف الرئيسي"))
        if is_global_admin(chat_id) and not is_super_admin(chat_id) and not testing_mode.get(chat_id):
            markup.add(KeyboardButton("🛡️ لوحة المشرف العام"))
        
        if is_any_admin(chat_id) and not testing_mode.get(chat_id):
            markup.add(KeyboardButton("👤 عرض كمستخدم"))
    ...
    if path_str == "GLOBAL_ADMIN_PANEL":
        ...
        markup.add("🔙 الرجوع للقائمة الرئيسية")
        bot.send_message(chat_id, "🛡️ *لوحة المشرف العام:*\nصلاحياتك الحالية:", reply_markup=markup, parse_mode="Markdown")
        return
    ...
    if path_str == "STUDENT_FEATURES":
        ...
        markup.add(KeyboardButton("⭐ ملفاتي المفضلة"))
        markup.add("🔙 الرجوع للقائمة الرئيسية")
    ...
    # [4] الأقسام والمجلدات الفعلية (تصاعدياً ومنظماً)
    ...
    # الترتيب التصاعدي للملفات القديمة (1, 2, 3...) بناءً على المعرف _id
    ...
    if path_str in ACADEMIC_STRUCTURE.keys(): markup.add("🔙 الرجوع للقائمة الرئيسية")
    ...
    if has_permission(chat_id, path_str):
        ...
        if current_menu is None: # للمجلدات الديناميكية
...
def send_file_to_user(chat_id, res, has_perm):
    try:
        ...
        markup.add(InlineKeyboardButton("💡 ملفات", callback_data=f"rl_{file_id_str}"), InlineKeyboardButton("⭐ تقييم", callback_data=f"rt_{file_id_str}"))
...
def universal_handler(message):
    ...
    # [ملاحة القوائم المباشرة الثابتة]
    main_nav = ["🔝 القائمة الرئيسية", "🔙 الرجوع للقائمة السابقة", "🔙 الرجوع للقائمة الرئيسية", "🌟 ميزات الطالب", "📖 دليل القسم", "⭐ ملفاتي المفضلة", "📞 التواصل مع المشرف العام", "👑 لوحة المشرف الرئيسي", "🛡️ لوحة المشرف العام", "👥 إدارة المشرفين", "🔑 صلاحيات المشرفين"] + list(ACADEMIC_STRUCTURE.keys())
    ...
    if text in main_nav:
        ...
        elif text == "👑 لوحة المشرف الرئيسي" and is_super_admin(chat_id): user_path[chat_id] = ["SUPER_ADMIN_PANEL"]
        elif text == "🛡️ لوحة المشرف العام" and is_global_admin(chat_id): user_path[chat_id] = ["GLOBAL_ADMIN_PANEL"]
        elif text == "👥 إدارة المشرفين" and is_super_admin(chat_id): user_path[chat_id] = ["MANAGE_ADMINS"]
        elif text == "🔑 صلاحيات المشرفين" and is_super_admin(chat_id): user_path[chat_id] = ["ADMIN_PERMISSIONS"]
        elif text == "📖 دليل القسم": 
            bot.send_message(chat_id, "📖 *دليل قسم الذكاء الاصطناعي وعلوم البيانات*\nسيتم إدراج الخطة الدراسية هنا.", parse_mode="Markdown"); return
    ...
    if text == "اللجنة العلمية" and path_str == "🌱 مستوى أول":
        sci_text = (
            ...
            "تتقدم الدفعة الثانية لقسم الذكاء الاصطناعي وعلوم البيانات بخالص الشكر والتقدير للإدارة وأعضاء اللجنة العلمية على جهودهم المبذولة والمختلفة في ترتيب وتوفير المصادر الدراسية للطلاب.\n\n"
            "👤 إدارة الدفعة:\n"
        ...
        bot.send_message(chat_id, sci_text); return

    # [الرفع الدفعي والفردي]
    if settings.get("emergency_flags", {}).get("upload", False) and not is_any_admin(chat_id):
        if message.content_type in ['document', 'photo', 'video', 'audio']: bot.send_message(chat_id, "🚧 الرفع معطل حالياً."); return
    ...
    # [إيقاف البوت بالكامل للطوارئ]
    if settings.get("status") == "inactive" and not is_any_admin(chat_id):
        bot.send_message(chat_id, "🚧 البوت تحت الصيانة. نعود قريباً."); return

    # [الأوامر الإدارية الشاملة - تعمل عالمياً]
    if text == "➕ إضافة مشرف عام" and is_super_admin(chat_id):
        reset_modes(chat_id); admin_action_mode[chat_id] = "add_glb"
        bot.send_message(chat_id, "أرسل الآيدي الرقمي للمشرف:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
    ...
    if text == "➖ حذف مشرف" and is_super_admin(chat_id):
    ...
    if text == "🚫 تعطيل مشرف" and is_super_admin(chat_id):
        reset_modes(chat_id); admin_action_mode[chat_id] = "deac_adm"
        bot.send_message(chat_id, "أرسل الآيدي لتعطيله:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
    if mode == "deac_adm" and text and is_super_admin(chat_id):
        try:
            tid = int(text.strip())
            if tid != SUPER_ADMIN_ID: admins_col.update_one({"id": tid}, {"$set": {"active": False}})
            bot.send_message(chat_id, "✅ تم التعطيل."); reset_modes(chat_id); show_menu(chat_id)
        except: bot.send_message(chat_id, "❌ أرقام فقط.")
        return

    if text == "✅ تفعيل مشرف" and is_super_admin(chat_id):
        reset_modes(chat_id); admin_action_mode[chat_id] = "ac_adm"
        bot.send_message(chat_id, "أرسل الآيدي لتفعيله:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
    if mode == "ac_adm" and text and is_super_admin(chat_id):
        try:
            tid = int(text.strip())
            admins_col.update_one({"id": tid}, {"$set": {"active": True}})
            bot.send_message(chat_id, "✅ تم التفعيل."); reset_modes(chat_id); show_menu(chat_id)
        except: bot.send_message(chat_id, "❌ أرقام فقط.")
        return

    if text == "🔍 البحث عن مشرف" and is_super_admin(chat_id):
    ...
    if mode == "srch_adm" and text and is_super_admin(chat_id):
        try:
            tid = int(text.strip())
            adm = admins_col.find_one({"id": tid})
            if adm: bot.send_message(chat_id, f"👤 نوعه: {adm.get('type')}\nنشط: {adm.get('active', True)}\nصلاحياته: {adm.get('permissions', [])}")
            else: bot.send_message(chat_id, "❌ غير موجود."); reset_modes(chat_id); show_menu(chat_id)
        except: bot.send_message(chat_id, "❌ أرقام فقط.")
        return

    if text == "🟢 منح صلاحية محددة" and is_super_admin(chat_id):
    ...
    if mode == "gnt_prm1" and text and is_super_admin(chat_id):
        try:
            tid = int(text.strip()); action_payload[chat_id] = tid; admin_action_mode[chat_id] = "gnt_prm2"
            m = ReplyKeyboardMarkup(resize_keyboard=True).add("إعلانات", "إحصائيات", "طوارئ", "تعديل نصوص", "أرشفة").add("🛑 إلغاء الأمر")
            bot.send_message(chat_id, "اختر الصلاحية:", reply_markup=m)
        except: bot.send_message(chat_id, "❌ أرقام فقط.")
        return
    if mode == "gnt_prm2" and text and is_super_admin(chat_id):
        p_map = {"إعلانات":"broadcast", "إحصائيات":"stats", "طوارئ":"emergency", "تعديل نصوص":"texts", "أرشفة":"archives"}
        if text in p_map:
            admins_col.update_one({"id": action_payload.get(chat_id)}, {"$addToSet": {"permissions": p_map[text]}})
            bot.send_message(chat_id, "✅ تم المنح."); reset_modes(chat_id); show_menu(chat_id); return

    if text == "🔴 سحب صلاحية محددة" and is_super_admin(chat_id):
        reset_modes(chat_id); admin_action_mode[chat_id] = "rvk_prm1"
        bot.send_message(chat_id, "أرسل آيدي المشرف:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
    if mode == "rvk_prm1" and text and is_super_admin(chat_id):
        try:
            tid = int(text.strip()); action_payload[chat_id] = tid; admin_action_mode[chat_id] = "rvk_prm2"
            m = ReplyKeyboardMarkup(resize_keyboard=True).add("إعلانات", "إحصائيات", "طوارئ", "تعديل نصوص", "أرشفة").add("🛑 إلغاء الأمر")
            bot.send_message(chat_id, "اختر الصلاحية لسحبها:", reply_markup=m)
        except: bot.send_message(chat_id, "❌ أرقام فقط.")
        return
    if mode == "rvk_prm2" and text and is_super_admin(chat_id):
        p_map = {"إعلانات":"broadcast", "إحصائيات":"stats", "طوارئ":"emergency", "تعديل نصوص":"texts", "أرشفة":"archives"}
        if text in p_map:
            admins_col.update_one({"id": action_payload.get(chat_id)}, {"$pull": {"permissions": p_map[text]}})
            bot.send_message(chat_id, "✅ تم السحب."); reset_modes(chat_id); show_menu(chat_id); return

    if text == "📋 عرض صلاحيات المشرف" and is_super_admin(chat_id):
        reset_modes(chat_id); admin_action_mode[chat_id] = "vw_prms"
        bot.send_message(chat_id, "أرسل الآيدي:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
    if mode == "vw_prms" and text and is_super_admin(chat_id):
        try:
            adm = admins_col.find_one({"id": int(text.strip())})
            bot.send_message(chat_id, f"صلاحياته: {adm.get('permissions', [])}" if adm else "❌ غير موجود."); reset_modes(chat_id); show_menu(chat_id)
        except: bot.send_message(chat_id, "❌ أرقام فقط."); return

    if text == "🚫 حظر مستخدم" and is_super_admin(chat_id):
        reset_modes(chat_id); admin_action_mode[chat_id] = "blk_usr"
        bot.send_message(chat_id, "🚫 أرسل الآيدي:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
    if mode == "blk_usr" and text and is_super_admin(chat_id):
        try:
            tid = int(text.strip())
            if tid == SUPER_ADMIN_ID: bot.send_message(chat_id, "❌ لا يمكن حظره.")
            else:
    ...
    if text == "✏️ تعديل نصوص البوت" and (is_super_admin(chat_id) or "texts" in get_admin_permissions(chat_id)):
        reset_modes(chat_id)
    ...
    # [ميزات الطالب والإعلانات]
    if text == "📢 إدارة الإعلانات" and (is_super_admin(chat_id) or "broadcast" in get_admin_permissions(chat_id)):
        reset_modes(chat_id); broadcast_mode[chat_id] = True
        bot.send_message(chat_id, "📢 أرسل الإعلان للطلاب:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
    if broadcast_mode.get(chat_id):
        broadcast_mode[chat_id] = False
        settings_col.update_one({}, {"$set": {"last_announcement": text if text else "مرفق إعلاني"}}, upsert=True)
        bot.send_message(chat_id, "⏳ جاري البث...")
        b_succ = 0
        for stu in list(users_col.find()):
            try: bot.copy_message(stu['chat_id'], chat_id, message.message_id); b_succ += 1
            except: pass
        bot.send_message(chat_id, f"📢 تم إيصال الرسالة إلى {b_succ} طالب."); show_menu(chat_id); return

    if text == "📢 إعلانات الدفعة":
        ann = settings_col.find_one({}).get("last_announcement", "لا إعلانات.")
        bot.send_message(chat_id, f"📢 *إعلان الدفعة:*\n\n{ann}", parse_mode="Markdown"); return

    if text == "🤖 المساعد الذكي (AI)":
        if settings_col.find_one({}).get("emergency_flags", {}).get("ai", False) and not is_any_admin(chat_id):
            bot.send_message(chat_id, "🚧 معطل للصيانة."); return
        reset_modes(chat_id); admin_action_mode[chat_id] = "ai_chat"
        if chat_id not in ai_memory: ai_memory[chat_id] = []
        bot.send_message(chat_id, "🤖 اسأل المساعد:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
    ...
    if text == "🔍 بحث عن ملف":
        if settings_col.find_one({}).get("emergency_flags", {}).get("search", False) and not is_any_admin(chat_id):
            bot.send_message(chat_id, "🚧 البحث معطل."); return
    ...
    if text == "⏰ تذكير شخصي":
        ...
        bot.send_message(chat_id, "⏰ موضوع التذكير؟", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
    ...
    if text == "🔥 الملفات الأكثر شعبية":
        ...
        else: bot.send_message(chat_id, "لا إحصائيات."); return
    if text == "🆕 تحديثات اليوم":
        ...
        else: bot.send_message(chat_id, "لا ملفات جديدة."); return
    ...
    if text in ["🔔 تفعيل الإشعارات", "🔕 إلغاء الإشعارات"]:
        ...
        bot.send_message(chat_id, "✅ تم التحديث."); show_menu(chat_id); return

    # [أزرار لوحة المشرف الشاملة]
    ...
    if text == "⭐️ التقييمات" and (is_super_admin(chat_id) or "courses_stats" in get_admin_permissions(chat_id)):
        ...
        for r in top:
            ...
            if f: msg += f"• {f['name']} | {r['avg']:.1f} ({r['cnt']} أصوات)\n"
        bot.send_message(chat_id, msg if top else "لا تقييمات.", parse_mode="Markdown"); return
    ...
    if text == "📊 إحصائيات المقررات" and (is_super_admin(chat_id) or "courses_stats" in get_admin_permissions(chat_id)):
        ...
        bot.send_message(chat_id, msg if stats else "لا إحصائيات.", parse_mode="Markdown"); return

    if text == "💾 النسخ الاحتياطي اليدوي" and (is_super_admin(chat_id) or "backup" in get_admin_permissions(chat_id)):
        ...
        bio.name = f"DB_Backup.json"
        bot.send_document(chat_id, bio, caption="💾 نسخة JSON."); return

    if text == "📝 سجل العمليات" and (is_super_admin(chat_id) or "logs" in get_admin_permissions(chat_id)):
        logs = list(action_logs_col.find().sort("timestamp", -1).limit(20))
        msg = "📝 *سجل العمليات:*\n"
        for lg in logs: msg += f"• {lg['timestamp'].strftime('%m-%d %H:%M')} | {lg.get('admin_name','-')} | {lg['action']}\n"
        bot.send_message(chat_id, msg if logs else "السجل فارغ.", parse_mode="Markdown"); return

    if text == "📊 نشاط المشرفين" and is_super_admin(chat_id):
        logs = list(action_logs_col.aggregate([{"$group": {"_id": "$admin_name", "count": {"$sum": 1}}}]))
        msg = "📊 *نشاط المشرفين:*\n"
        for l in logs: msg += f"• {l['_id']}: {l['count']} عملية\n"
        bot.send_message(chat_id, msg if logs else "لا نشاط.", parse_mode="Markdown"); return

    if text == "🔍 كشف الملفات المكررة" and is_super_admin(chat_id):
        dups = list(files_col.aggregate([{"$group": {"_id": "$file_id", "count": {"$sum": 1}, "names": {"$push": "$name"}}}, {"$match": {"count": {"$gt": 1}}}]))
        msg = "🔍 *ملفات مكررة (نفس ID):*\n"
        for d in dups[:10]: msg += f"• التكرار: {d['count']} | الاسم: {d['names'][0]}\n"
        bot.send_message(chat_id, msg if dups else "✅ لا تكرار.", parse_mode="Markdown"); return

    if text == "🚨 وضع الطوارئ" and (is_super_admin(chat_id) or "emergency" in get_admin_permissions(chat_id)):
        flags = settings_col.find_one({}).get("emergency_flags", {})
    ...
    if text in ["🟢 AI", "🔴 AI", "🟢 الرفع", "🔴 الرفع", "🟢 البحث", "🔴 البحث"] and is_any_admin(chat_id):
        key = "ai" if "AI" in text else ("upload" if "الرفع" in text else "search")
        cur = settings_col.find_one({}).get("emergency_flags", {}).get(key, False)
        settings_col.update_one({}, {"$set": {f"emergency_flags.{key}": not cur}})
        bot.send_message(chat_id, f"✅ تم التبديل."); show_menu(chat_id); return
    if text == "🛑 إيقاف البوت كلياً" and is_any_admin(chat_id):
        settings_col.update_one({}, {"$set": {"status": "inactive"}})
        bot.send_message(chat_id, "✅ تم إيقاف البوت للطلاب."); show_menu(chat_id); return

    if text == "🏷️ إدارة الأرشفة" and (is_super_admin(chat_id) or "archives" in get_admin_permissions(chat_id)):
        archive_markup = ReplyKeyboardMarkup(resize_keyboard=True).add("📋 عرض الهاشتاجات", "🗑️ حذف هاشتاج", "🔙 الرجوع للقائمة السابقة")
        bot.send_message(chat_id, "🏷️ *إدارة الأرشفة*", reply_markup=archive_markup, parse_mode="Markdown"); return
    if text == "📋 عرض الهاشتاجات" and is_any_admin(chat_id):
        active_tags = list(hashtags_col.find())
        h_msg = "🏷️ *الهاشتاجات النشطة:*\n" + "".join([f"🔸 {t['tag']} ⇦ {t['path'].split(' > ')[-1]}\n" for t in active_tags])
        bot.send_message(chat_id, h_msg if active_tags else "لا هاشتاجات.", parse_mode="Markdown"); return
    if text == "🗑️ حذف هاشتاج" and is_any_admin(chat_id):
        reset_modes(chat_id); admin_action_mode[chat_id] = "del_hashtag"
        bot.send_message(chat_id, "أرسل الهاشتاج:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
    if mode == "del_hashtag" and text:
        ...
        bot.send_message(chat_id, "✅ تم الحذف."); reset_modes(chat_id); show_menu(chat_id); return

    # [تفعيل عرض المشرف كمستخدم]
    ...
    if mode == "ask_path_admin_id" and text and is_super_admin(chat_id):
        try:
            ...
            bot.send_message(chat_id, f"✅ تم تقييد المشرف بنجاح."); reset_modes(chat_id); show_menu(chat_id)
        except: bot.send_message(chat_id, "❌ أرقام فقط.")
        return

    # [العمليات الإدارية داخل الأقسام (إنشاء، تعديل، حذف، ترتيب)]
    if has_permission(chat_id, path_str) and path_str and path_str not in ["SUPER_ADMIN_PANEL", "GLOBAL_ADMIN_PANEL", "STUDENT_FEATURES", "FAVORITES", "MANAGE_ADMINS", "ADMIN_PERMISSIONS"]:
        ...
        if text == "✏️ إعادة تسمية هذا القسم":
            ...
            bot.send_message(chat_id, "✏️ أرسل الاسم الجديد (سيتحدث المسار بأمان لجميع الملفات):", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("🛑 إلغاء الأمر")); return
        ...
        if text in ["🔼 نقل مجلد للأعلى", "🔽 نقل مجلد للأسفل"]:
            ...
            if fld:
                inc = -1 if "للأعلى" in text else 1
                folders_col.update_one({"_id": fld["_id"]}, {"$inc": {"sort_order": inc}})
                user_path[chat_id].pop()
                bot.send_message(chat_id, "✅ تم تغيير الترتيب. رجعنا خطوة للخلف لتحديث العرض.")
            show_menu(chat_id); return
    ...
    if mode == "rn_fld" and text:
        ...
        # [تحديث متكرر (Recursive) للمسارات للحفاظ على الملفات القديمة]
    ...
    if mode == "add_hashtag" and text and is_super_admin(chat_id):
        ...
        bot.send_message(chat_id, f"✅ تم ربط الهاشتاج."); reset_modes(chat_id); show_menu(chat_id); return
    ...
    # [فتح الملفات والمجلدات الديناميكية (Dynamic Paths)]
...
# ==========================================
@bot.callback_query_handler(func=lambda call: call.data.startswith(('rn_', 'rp_', 'dl_', 'mv_', 'up_', 'dn_', 'pn_', 'fv_', 'rt_', 'str_')))
def handle_inline_callbacks(call):
    chat_id = call.message.chat.id
    try: action, obj_id = call.data.split('_', 1)
    except: return

    # [المفضلة والتقييم للطلاب]
    if action == 'fv':
    ...
    if action == 'dl':
        ...
        bot.send_message(chat_id, "📦 تصفح الأقسام للوصول لموقع النقل واضغط التأكيد.")
...
@app.route("/")
def index_home_route(): return "Bot V5 LMS is RUNNING 🚀", 200
