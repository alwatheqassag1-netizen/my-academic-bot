...
cursor.execute('''CREATE TABLE IF NOT EXISTS items 
                  (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, type TEXT, file_id TEXT, parent_id INTEGER)''')
cursor.execute('CREATE TABLE IF NOT EXISTS users (chat_id INTEGER PRIMARY KEY)')

def build_initial_tree():
    cursor.execute('SELECT COUNT(*) FROM items')
    if cursor.fetchone()[0] == 0:
        levels = ["مستوى أول", "مستوى ثاني", "مستوى ثالث", "مستوى رابع"]
        for l in levels: 
            cursor.execute('INSERT INTO items (name, type, parent_id) VALUES (?, "مجلد", 0)', (l,))
        
        for parent_level_id in [1, 2, 3, 4]:
            cursor.execute('INSERT INTO items (name, type, parent_id) VALUES ("ترم أول", "مجلد", ?)', (parent_level_id,))
            cursor.execute('INSERT INTO items (name, type, parent_id) VALUES ("ترم ثاني", "مجلد", ?)', (parent_level_id,))
        
        subjects = ["ثقافة إسلامية", "لغة عربية 2", "لغة إنجليزية 2", "تفاضل وتكامل 2", "مقدمة في علوم البيانات", "برمجة حاسوب", "رياضيات متقطعة"]
        for s in subjects: 
            cursor.execute('INSERT INTO items (name, type, parent_id) VALUES (?, "مجلد", 6)', (s,))
        
        for i in [13, 14, 15]:
            cursor.execute('INSERT INTO items (name, type, parent_id) VALUES ("محاضرات وملخصات", "مجلد", ?)', (i,))
            cursor.execute('INSERT INTO items (name, type, parent_id) VALUES ("نماذج اختبارات", "مجلد", ?)', (i,))
